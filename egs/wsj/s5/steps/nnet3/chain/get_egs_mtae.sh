...
frame_weight_dae=1.0  # Sets weight of output_dae loss
frame_subsampling_factor_dae=3 # Set frame_subsampleing_factor for output_dae
frame_weight_dspae=1.0  # Sets weight of output_dspae loss
frame_subsampling_factor_dspae=3 # Set frame_subsampleing_factor for output_dspae
...
if [ $# != 6 ]; then
  echo "Usage: $0 [opts] <data> <chain-dir> <lattice-dir> <targets-scp-dae> <targets-scp-dspae> <egs-dir>"
  echo " e.g.: $0 data/train exp/tri4_nnet exp/tri3_lats data/train/feats.scp data/train/feats2.scp exp/tri4_nnet/egs"
  echo ""
...
echo $chaindir
echo $latdir
echo $targets_scp_dae
echo $targets_scp_dspae
echo $dir
...
for f in $data/feats.scp $latdir/lat.1.gz $latdir/final.mdl \
         $chaindir/{0.trans_mdl,tree,normalization.fst} $targets_scp_dae $targets_scp_dspae $extra_files; do
...
if [ $target_type == "dense" ]; then
  num_targets=$(feat-to-dim "scp:$targets_scp_dae" - 2>/dev/null) || exit 1
else
...
case $target_type in
  "dense") 
    ...
    targets_dae="scp:$targets_scp_dae"
    valid_targets_dae="scp:$targets_scp_dae"
    train_subset_targets_dae="scp:$targets_scp_dae"

    targets_dspae="scp:$targets_scp_dspae"
    valid_targets_dspae="scp:$targets_scp_dspae"
    train_subset_targets_dspae="scp:$targets_scp_dspae"
    ;;
  # "sparse")
  ...
  (
    ...
    $cmd $dir/log/create_valid_subset.log \
      ...
      chain-get-supervision $chain_supervision_all_opts $chaindir/tree $chaindir/0.trans_mdl \
        ark:- ark:- \| \
      nnet3-chain-get-egs-mtae $ivector_opts --srand=$srand --num-targets=$num_targets \
...
if [ $stage -le 4 ]; then
  ...
  $cmd --max-jobs-run $max_jobs_run JOB=1:$nj $dir/log/get_egs.JOB.log \
    ...
    chain-get-supervision $chain_supervision_all_opts \
      $chaindir/tree $chaindir/0.trans_mdl ark:- ark:- \| \
    nnet3-chain-get-egs-mtae $ivector_opts --srand=\$[JOB+$srand] $egs_opts --num-targets=$num_targets \
      --frame-weight-dae=$frame_weight_dae --frame-subsampling-factor-dae=$frame_subsampling_factor_dae \
      --frame-weight-dspae=$frame_weight_dspae --frame-subsampling-factor-dspae=$frame_subsampling_factor_dspae \
      --length-tolerance=$length_tolerance \
