...
def get_args():
    ...
    # parser.add_argument("--egs.target-clean", type=str, dest='target_clean',
    #                     default="false",
    ...
    parser.add_argument("--egs.frame-weight-dae", type=float, dest='frame_weight_dae',
                        default=1.0,
                        help="""For mtae training with multi-condition data. Set 
                        weight of output_dae loss""")
    parser.add_argument("--egs.frame-subsampling-factor-dae", type=int, dest='frame_subsampling_factor_dae',
                        default=3,
                        help="""For mtae training with multi-condition data. Set 
                        frame_subsampling_factor_dae of output_dae loss""")
    parser.add_argument("--egs.frame-weight-dspae", type=float, dest='frame_weight_dspae',
                        default=1.0,
                        help="""For mtae training with multi-condition data. Set 
                        weight of output_dspae loss""")
    parser.add_argument("--egs.frame-subsampling-factor-dspae", type=int, dest='frame_subsampling_factor_dspae',
                        default=3,
                        help="""For mtae training with multi-condition data. Set 
                        frame_subsampling_factor_dspae of output_dspae loss""")
    ...
    parser.add_argument("--target_scp_dae", type=str, required=True,
                        help="Directory with target_scp_dar for MTAE")
    parser.add_argument("--target_scp_dspae", type=str, required=True,
                        help="Directory with target_scp_dspae for MTAE")
...
def train(args, run_opts):
    ...
    if ((args.stage <= -3) and args.egs_dir is None):
        ...
        common_lib.execute_command(
            """steps/nnet3/chain/get_egs_mtae.sh {egs_opts} \
                --cmd "{command}" \
                --cmvn-opts "{cmvn_opts}" \
                ...
                --frame-weight-dae {frame_weight_dae} \
                --frame-subsampling-factor-dae {frame_subsampling_factor_dae} \
                --frame-weight-dspae {frame_weight_dspae} \
                --frame-subsampling-factor-dspae {frame_subsampling_factor_dspae} \
                --srand {srand} \
                --generate-egs-scp true \
                {data_dir} {dir} {lat_dir} {target_scp_dae} {target_scp_dspae} {egs_dir}""".format(
                ...
                frame_weight_dae=args.frame_weight_dae,
                frame_subsampling_factor_dae=args.frame_subsampling_factor_dae,
                frame_weight_dspae=args.frame_weight_dspae,
                frame_subsampling_factor_dspae=args.frame_subsampling_factor_dspae,
                srand=args.srand,
                data_dir=args.feat_dir, dir=args.dir, lat_dir=args.lat_dir,
                target_scp_dae=args.target_scp_dae, target_scp_dspae=args.target_scp_dspae, egs_dir=default_egs_dir))
