...
if [ $stage -le 2 ] && [ -f data/$mic/${train_set}_sp_hires_${hires_suffix}/feats.scp ]; then
  echo "$0: data/$mic/${train_set}_sp_hires_${hires_suffix}/feats.scp already exists."
...
if [ $stage -le 2 ]; then
  ...
  mfccdir=data/$mic/${train_set}_sp_hires_${hires_suffix}/data
  ...
  for datadir in ${train_set}_sp dev eval; do
    utils/copy_data_dir.sh data/$mic/$datadir data/$mic/${datadir}_hires_${hires_suffix}
  done

  # do volume-perturbation on the training data prior to extracting hires
  # features; this helps make trained nnets more invariant to test data volume.
  utils/data/perturb_data_dir_volume.sh data/$mic/${train_set}_sp_hires_${hires_suffix}

  for datadir in ${train_set}_sp dev eval; do
    steps/make_mfcc.sh --nj $nj --mfcc-config conf/mfcc_hires$hires_suffix.conf \
      --cmd "$train_cmd" data/$mic/${datadir}_hires_${hires_suffix}
    steps/compute_cmvn_stats.sh data/$mic/${datadir}_hires_${hires_suffix}
    utils/fix_data_dir.sh data/$mic/${datadir}_hires_${hires_suffix}
  done
fi
...
if [ $stage -le 3 ]; then
  ...
  utils/data/combine_short_segments.sh \
     data/${mic}/${train_set}_sp_hires_${hires_suffix} $min_seg_len data/${mic}/${train_set}_sp_hires_${hires_suffix}_comb

  # just copy over the CMVN to avoid having to recompute it.
  cp data/${mic}/${train_set}_sp_hires_${hires_suffix}/cmvn.scp data/${mic}/${train_set}_sp_hires_${hires_suffix}_comb/
  utils/fix_data_dir.sh data/${mic}/${train_set}_sp_hires_${hires_suffix}_comb/
fi
...
if [ $stage -le 4 ]; then
  ...
  utils/data/subset_data_dir.sh --utt-list data/${mic}/${train_set}/feats.scp \
          data/${mic}/${train_set}_sp_hires_${hires_suffix} $temp_data_root/${train_set}_hires_${hires_suffix}

  # note: essentially all the original segments should be in the hires data.
  n1=$(wc -l <data/${mic}/${train_set}/feats.scp)
  n2=$(wc -l <$temp_data_root/${train_set}_hires_${hires_suffix}/feats.scp)
  if [ $n1 != $n1 ]; then
  ...
  case $ivector_transform_type in
    lda)
      ...
      steps/train_lda_mllt.sh --cmd "$train_cmd" --num-iters 7 --mllt-iters "2 4 6" \
        ...
        3000 10000 $temp_data_root/${train_set}_hires_${hires_suffix} data/lang \
    ...
    pca)
      ...
      steps/online/nnet2/get_pca_transform.sh --cmd "$train_cmd" \
        ...
        $temp_data_root/${train_set}_hires_${hires_suffix} \
...
if [ $stage -le 5 ]; then
  ...
  num_utts_total=$(wc -l <data/$mic/${train_set}_sp_hires_${hires_suffix}/utt2spk)
  num_utts=$[$num_utts_total/4]
  utils/data/subset_data_dir.sh data/$mic/${train_set}_sp_hires_${hires_suffix} \
      $num_utts ${temp_data_root}/${train_set}_sp_hires_${hires_suffix}_subset
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 30 \
    ...
    ${temp_data_root}/${train_set}_sp_hires_${hires_suffix}_subset 512 \
...
if [ $stage -le 6 ]; then
  ...
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj 10 \
    data/$mic/${train_set}_sp_hires_${hires_suffix} exp/$mic/nnet3${nnet3_affix}/diag_ubm exp/$mic/nnet3${nnet3_affix}/extractor || exit 1;
fi
...
if [ $stage -le 7 ]; then
  ...
  ivectordir=exp/$mic/nnet3${nnet3_affix}/ivectors_${train_set}_sp_hires_${hires_suffix}_comb
  ...
  utils/data/modify_speaker_info.sh --utts-per-spk-max 2 \
    data/${mic}/${train_set}_sp_hires_${hires_suffix}_comb ${temp_data_root}/${train_set}_sp_hires_${hires_suffix}_comb_max2

  steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj $nj \
    ${temp_data_root}/${train_set}_sp_hires_${hires_suffix}_comb_max2 \
  ...
  for data in dev eval; do
    steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj "$nj" \
      data/${mic}/${data}_hires_${hires_suffix} exp/$mic/nnet3${nnet3_affix}/extractor \
      exp/$mic/nnet3${nnet3_affix}/ivectors_${data}_hires_${hires_suffix}
  done
fi
