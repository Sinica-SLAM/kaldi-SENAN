...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "\n"
        "Usage:  nnet3-chain-get-egs-dcae [options] [<normalization-fst>] <features-rspecifier> "
        "<chain-supervision-rspecifier> <egs-wspecifier>\n"
        "\n"
        "An example [where $feats expands to the actual features]:\n"
        "chain-get-supervision [args] | \\\n"
        "  nnet3-chain-get-egs-dcae --num-targets=26 --left-context=25 --right-context=9 --num-frames=150,100,90 dir/normalization.fst \\\n"
    ...
    po.Register("target-clean", &target_clean,
    ...
    std::string normalization_fst_rxfilename, 
        ...
        target_matrix_rspecifier,
        examples_wspecifier;

    if (po.NumArgs() == 4) {
      feature_rspecifier = po.GetArg(1);
      supervision_rspecifier = po.GetArg(2);
      target_matrix_rspecifier = po.GetArg(3),  // get-egs-dense-targets
    ...
    RandomAccessBaseFloatMatrixReader matrix_reader(
        target_matrix_rspecifier);  // get-egs-dense-targets
