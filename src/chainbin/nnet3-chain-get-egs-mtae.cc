...
static bool ProcessFile(const TransitionModel *trans_mdl,
                        ...
                        int32 num_targets,
                        const std::vector<BaseFloat> &frame_weight_list,
                        const std::vector<int32> &frame_subsampling_factor_list,
                        const std::vector<Matrix<BaseFloat>> &targets_list,
                        const std::vector<std::string> layer_name_list,
  ...
  //   return false;
  // }
  if (targets_list[0].NumRows() == 0) return false;
  KALDI_ASSERT(num_targets < 0 || targets_list[0].NumCols() == num_targets);
  ...
  for (size_t c = 0; c < chunks.size(); c++) {
    ...
    std::vector<BaseFloat> start_frame_subsampled_list;
    std::vector<BaseFloat> num_frames_subsampled_list;
    for (int32 i = 0; i < frame_subsampling_factor_list.size(); i++) {
      start_frame_subsampled_list.push_back(chunk.first_frame / frame_subsampling_factor_list[i]);
      num_frames_subsampled_list.push_back(chunk.num_frames / frame_subsampling_factor_list[i]);
    }
    ...
    nnet_chain_eg.outputs_ae.resize(targets_list.size());
    for (int32 l=0; l<targets_list.size(); l++) {
      Matrix<BaseFloat> targets = targets_list[l];

      // KALDI_ASSERT(start_frame_subsampled_list[l] + num_frames_subsampled_list[l] - 1 <
      //            targets.NumRows());
      
      // add the labels.
      Matrix<BaseFloat> targets_part(num_frames_subsampled_list[l], targets.NumCols());
      for (int32 i = 0; i < num_frames_subsampled_list[l]; i++) {
        // Copy the i^th row of the target matrix from the (t+i)^th row of the
        // input targets matrix
        int32 t = i + start_frame_subsampled_list[l];
      ...
      NnetIo dcae_io(layer_name_list[l], 0, targets_part, frame_weight_list[l], frame_subsampling_factor_list[l]);
      nnet_chain_eg.outputs_ae[l].Swap(&dcae_io);
    }
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "\n"
        "Usage:  nnet3-chain-get-egs-mtae [options] [<normalization-fst>] <features-rspecifier> "
        "<chain-supervision-rspecifier> <egs-wspecifier>\n"
        "\n"
        "An example [where $feats expands to the actual features]:\n"
        "chain-get-supervision [args] | \\\n"
        "  nnet3-chain-get-egs-mtae --num-targets=26 --left-context=25 --right-context=9 --num-frames=150,100,90 dir/normalization.fst \\\n"
    ...
    BaseFloat frame_weight_dae = 1.0, frame_weight_dspae = 1.0;
    int32 frame_subsampling_factor_dae = 3; // compatible with orginal chain model 
    int32 frame_subsampling_factor_dspae = 3; // compatible with orginal chain model 
    ...
    po.Register("frame-weight-dae", &frame_weight_dae,
                "For mtae training,"
                "Set the weight between decoder loss and ASR loss (0~1)");
    po.Register("frame-subsampling-factor-dae", &frame_subsampling_factor_dae,
                "For mtae training,"
                "Set the frame-subsampling-factor for output_dae");
    po.Register("frame-weight-dspae", &frame_weight_dspae,
                "For mtae training,"
                "Set the weight between decoder loss and ASR loss (0~1)");
    po.Register("frame-subsampling-factor-dspae", &frame_subsampling_factor_dspae,
                "For mtae training,"
                "Set the frame-subsampling-factor for output_dspae");
    ...
    std::string normalization_fst_rxfilename, 
        feature_rspecifier,
        supervision_rspecifier,
        target_dae_matrix_rspecifier,
        target_dspae_matrix_rspecifier,
        examples_wspecifier;

    if (po.NumArgs() == 5) {
      feature_rspecifier = po.GetArg(1);
      supervision_rspecifier = po.GetArg(2);
      target_dae_matrix_rspecifier = po.GetArg(3),  // get-egs-dense-targets
      target_dspae_matrix_rspecifier = po.GetArg(4),  // get-egs-dense-targets
      ...
      target_dae_matrix_rspecifier = po.GetArg(4),  // get-egs-dense-targets
      target_dspae_matrix_rspecifier = po.GetArg(5),  // get-egs-dense-targets
    ...
    RandomAccessBaseFloatVectorReader deriv_weights_reader(
        deriv_weights_rspecifier);
    RandomAccessBaseFloatMatrixReader target_dae_matrix_reader(
        target_dae_matrix_rspecifier);  // get-egs-dense-targets
    RandomAccessBaseFloatMatrixReader target_dspae_matrix_reader(
        target_dspae_matrix_rspecifier);  // get-egs-dense-targets
    ...
    for (; !feat_reader.Done(); feat_reader.Next()) {
      ...
      if (!supervision_reader.HasKey(key)) {
        ...
        num_err++;
      } else if (!target_dae_matrix_reader.HasKey(target_key)) {  // get-egs-dense-targets
        KALDI_WARN << "No target matrix for key " << target_key;
        num_err++;
      } else {
        const chain::Supervision &supervision = supervision_reader.Value(key);
        const Matrix<BaseFloat> &target_dae_matrix = target_dae_matrix_reader.Value(target_key);  // get-egs-dense-targets
        const Matrix<BaseFloat> &target_dspae_matrix = target_dspae_matrix_reader.Value(target_key);  // get-egs-dense-targets
        ...
        std::vector<BaseFloat> frame_weight_list;
        frame_weight_list.push_back(frame_weight_dae);
        frame_weight_list.push_back(frame_weight_dspae);

        std::vector<int32> frame_subsampling_factor_list;
        frame_subsampling_factor_list.push_back(frame_subsampling_factor_dae);
        frame_subsampling_factor_list.push_back(frame_subsampling_factor_dspae);

        std::vector<Matrix<BaseFloat>> target_matrix_list;
        target_matrix_list.push_back(target_dae_matrix);
        target_matrix_list.push_back(target_dspae_matrix);

        std::vector<std::string> layer_name;
        layer_name.push_back("output-dae");
        layer_name.push_back("output-dspae");

        if (!ProcessFile(
                trans_mdl_ptr, normalization_fst, feats, online_ivector_feats,
                online_ivector_period, supervision, deriv_weights,
                supervision_length_tolerance, num_targets,
                frame_weight_list, frame_subsampling_factor_list, target_matrix_list, layer_name,
